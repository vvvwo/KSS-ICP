...
public:
	...
	vector<vector<double>> BallRegion_ReturnBox_Region(int boxIndex) {
		...
		double xmin = minXYZ[0] + (x_num - 1) * unitSize;
		double xmax = minXYZ[0] + x_num * unitSize;
		vector<double> xv;
		xv.push_back(xmin);
		xv.push_back(xmax);

		double ymin = minXYZ[1] + (y_num - 1) * unitSize;
		double ymax = minXYZ[1] + y_num * unitSize;
		vector<double> yv;
		yv.push_back(ymin);
		yv.push_back(ymax);

		double zmin = minXYZ[2] + (z_num - 1) * unitSize;
		double zmax = minXYZ[2] + z_num * unitSize;
		vector<double> zv;
		zv.push_back(zmin);
		zv.push_back(zmax);
...
#pragma omp parallel for
	...
	void BallRegion_BoxInput() {//Input points into the boxes
		...
		for (int i = 0; i < pointCloudData.size(); i++) {
			...
			if (index_Num >= squareBoxesCenterReal.size() || index_Num < 0) {
				cout << "Hello!" << endl;
